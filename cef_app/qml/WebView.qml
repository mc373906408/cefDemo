...
Item {
    ...
    QtObject {
        ...
        /* @id:作为消息id，递增，@request：使用FDJMsgId.id_xxx()获取，@msg：成功信息（一般为success） */
        ...
        /* @id:作为消息id，递增，@request：使用FDJMsgId.id_xxx()获取，@failureId:失败id（暂未规定），@msg：失败信息 */
...
//            console.log("name:"+name+" recv H5:"+request)
            ...
            if(json["Close"]!==undefined){
                ...
                callback(name,createSuccessMsg(str.id,request,cppCef.getCefClients(json["Close"])))
                ret={"OpenChildren":cppCef.getCefClients(fatherName),"BeName":name}
            ...
            if(json["GetConfig"]!==undefined){
                callback(name,createSuccessMsg(str.id,request,cppConfig.get(json["GetConfig"])))
            }
        }
    }
}
